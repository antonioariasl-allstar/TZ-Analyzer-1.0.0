...
def build_recent_contacts_section(
    df: pd.DataFrame,
    days: int = 5,
    max_rows: int = 200,
) -> str:
    """Genera la tabla de contactos recientes limitada a `days` días y `max_rows` filas."""
    ...
    try:
        ...
        start = max_dt - pd.Timedelta(days=days)
        ...
        r = r.sort_values("_dt", ascending=False).head(max_rows)
        ...
        out: list[str] = []
        out.append('<section id="contactos-recientes">')
        out.append(f'<h2>Contactos recientes (últimos {days} días)</h2>')
        out.append('<div class="tabla-scroll"><table class="tabla-compacta">')
        out.append('<thead><tr><th>Fecha/Hora</th><th>Tipo</th><th>Contacto</th><th>Duración</th></tr></thead><tbody>')
        out.extend(filas)
        out.append("</tbody></table></div>")
        out.append("</section>")
        return "\n".join(out)
